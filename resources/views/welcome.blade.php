        ...
        <div class="row">
            <div class="col-md-8 order-md-2 col-lg-9">
                <div class="container-fluid">
                    ...
                    <div class="row" id="products-wrapper">                        @foreach($products as $product)
...
<input type="checkbox" class="custom-control-input" name="filter[categories][]" id="category-{{ $category->id }}" value="{{ $category->id }}">                            <label class="custom-control-label" for="category-{{ $category->id }}">{{ $category->name }}</label>
                        </div>                    </div>
                @endforeach
                ...
                <div class="price-filter-control">
                    ...
                    <input type="number" class="form-control w-50 pull-right"  name ="filter[price_max]" value="150" id="price-max-control">
                </div>
                <input id="ex2" type="text" class="slider " value="50,150" data-slider-min="10" data-slider-max="200" data-slider-step="5" data-slider-value="[50,150]" data-value="50,150" style="display: none;">
                <div class="divider mt-5 mb-5 border-bottom border-secondary"></div>
                <a href="#" id="filter-button" class="btn btn-lg btn-block btn-primary mt-5">{{ __('shop.welcome.filter') }}</a>
            </form>
...
@endsection
@section('javascript')
    const storagePath = '{{ Vite::asset('public/storage') }}/';
    const defaultImage = '{{ $defaultImage }}';
@endsection
@section('js-files')
    <script src="{{ Vite::asset("resources/js/welcome.js") }}"></script>
@endsection